...
public class EmailMessage {
    ...
    /* SMTP-recipient, or contents of To-header, which could contain multiple recipients */
    ...
    public EmailMessage(String from, String[] to, String subject, String text,
        ...
        /* Remove whitespace */
        Sender = from.trim();
        Recipient = new String[to.length];
        System.arraycopy(to, 0, Recipient, 0, to.length);
    ...
    public boolean isValid() {
        ...
        for (int i = 0; i < Recipient.length; i++) {
            toat = Recipient[i].indexOf('@');
            if (toat < 1 || (Recipient[i].length() - toat) <= 1) {
                System.out.println("Recipient address is invalid");
                return false;
            }
            if (toat != Recipient[i].lastIndexOf('@')) {
                System.out.println("Recipient address is invalid");
                return false;
            }
        }
